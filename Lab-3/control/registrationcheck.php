...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($uname) || empty($email) || empty($fname) || empty($pass) || empty($cpass)) {
        $msg = "Enter your Details This fields are requied";
    } else if ((strlen($uname) < 5) || (!preg_match($pattern1, $uname))) {
        $msg = "username contain 5 characters and alpha numeric characters, period, dash or underscore";
    } else if ((strlen($pass) < 8) && (strlen($pass) < 8)) {
        $msg = "password contain 8 characters";
    } else if (strpos($email, "@") === false) {
        $msg = "Email address must contain @";
    } else if (!isset($_REQUEST["gender"])) {
        $msg = "select gender";
    } else if ($pass != $cpass) {
        $msg = "Password will be same";
